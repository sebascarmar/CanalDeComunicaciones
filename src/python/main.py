...
def main():
    ...
    (tf, filt, dot) = filtro_pulso(fc, fs, beta, OS, nbaud, Norm, fixed, NB, NBF, truncar_redondear, saturar_overflow,
                                   signed_unsigned, RRC, n_taps=0)
    ...
    rct, rcv, rc_dot = filtro_pulso(fc, fs, beta, OS, nbaud, Norm, fixed, NB, NBF, truncar_redondear, saturar_overflow, signed_unsigned, RRC=False, n_taps=len(h_rrc_rrc))
    ...
    while True:
        ...
        changes = beta != float(values['B']) or OS != int(values['OS']) or nbaud != int(values['N Bauds']) or  \
                Norm != values['Norm'] or RRC != values['RRC'] or NB != int(values['NB']) or NBF != int(values['NBF']) or \
                truncar_redondear != values['t/r'] or phase != int(values['PConst']) or saturar_overflow != values['s/oF'] or \
                signed_unsigned != values['sig/unsig'] or FB != int(values['FB']) or fixed != values['Fixed'] or \
        ...
        NBF = int(values['NBF']);
        RRC = values['RRC']
        truncar_redondear = values['t/r'];
        saturar_overflow = values['s/oF'];
        signed_unsigned = values['sig/unsig'];
        fixed = values['Fixed'];
        gauss = values['gauss']
        ...
        if changes:
            ...
            (tf, filt, dot) = filtro_pulso(fc, fs, beta, OS, nbaud, Norm, fixed, NB, NBF, truncar_redondear,
                                           saturar_overflow, signed_unsigned, RRC, n_taps=0)
